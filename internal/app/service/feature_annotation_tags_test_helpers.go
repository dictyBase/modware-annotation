...
// verifyProvidedTimestampsParams holds the parameters for the
// verifyProvidedTimestamps function.
type verifyProvidedTimestampsParams struct {
	params             *testParams
	result             *feature.FeatureAnnotation
	expectedTags       []*feature.TagPropertyCreate
...
// verifyTagTimestampsParams holds the parameters for the
// verifyTagTimestamps function.
type verifyTagTimestampsParams struct {
	params             *testParams
	result             *feature.FeatureAnnotation
	expectedTags       []*feature.TagPropertyCreate
	expectedTimestamps []time.Time
...
// testTimestampBehaviorParams holds the parameters for the
// testTimestampBehavior function.
type testTimestampBehaviorParams struct {
	params                 *testParams
	featureID              string
...
// verifyServiceTagRemovedParams holds the parameters for the
// verifyServiceTagRemoved function.
type verifyServiceTagRemovedParams struct {
	params           *testParams
	result           *feature.FeatureAnnotation
	removedTag       string
	removedValue     string
...
func testAddTagsDefaultTimestamps(params *testParams) {
	params.t.Helper()
	testTimestampBehavior(&testTimestampBehaviorParams{
		params:                 params,
		featureID:              "DDB_G0285505",
		withProvidedTimestamps: false,
		operation: func(
			featureID string,
			tags []*feature.TagPropertyCreate,
		) (*feature.FeatureAnnotation, error) {
...
func testAddTagsProvidedTimestamps(params *testParams) {
	...
	testTimestampBehavior(&testTimestampBehaviorParams{
		params:                 params,
		featureID:              "DDB_G0285506",
		withProvidedTimestamps: true,
		operation: func(featureID string, tags []*feature.TagPropertyCreate) (*feature.FeatureAnnotation, error) {
			addReq := createAddTagsServiceRequest(featureID, tags)
			return params.client.AddTags(params.ctx, addReq)
		},
	})
...
func verifyProvidedTimestamps(args *verifyProvidedTimestampsParams) {
	args.params.t.Helper()
	// Verify provided timestamps are preserved
	for idx, expectedTag := range args.expectedTags {
		pdx := slices.IndexFunc(
			args.result.Attributes.Properties,
			func(p *feature.TagProperty) bool {
				return p.Tag == expectedTag.Tag
		...
		args.params.assert.Equal(
			args.expectedTimestamps[idx].Truncate(time.Second),
			args.result.Attributes.Properties[pdx].CreatedAt.AsTime().
...
func verifyTagTimestamps(args *verifyTagTimestampsParams) {
	args.params.t.Helper()
	if args.autoGenerated {
		verifyAutoGeneratedTimestamps(
			args.params,
			args.result,
			args.expectedTags,
		)
	} else {
		verifyProvidedTimestamps(&verifyProvidedTimestampsParams{
			params:             args.params,
			result:             args.result,
			expectedTags:       args.expectedTags,
			expectedTimestamps: args.expectedTimestamps,
		})
	}
}
...
func testTimestampBehavior(args *testTimestampBehaviorParams) {
	args.params.t.Helper()

	// Create a feature first
	createReq := newTestFeature()
	createReq.Id = args.featureID
	created, err := args.params.client.CreateFeatureAnnotation(
		args.params.ctx,
		createReq,
	)
	args.params.assert.NoError(err, "should successfully create test feature")

	var newTags []*feature.TagPropertyCreate
	var expectedTimestamps []time.Time

	if args.withProvidedTimestamps {
	...
	result, err := args.operation(created.Id, newTags)
	args.params.assert.NoError(err, "should successfully perform tag operation")

	// Verify timestamps
	verifyTagTimestamps(&verifyTagTimestampsParams{
		params:             args.params,
		result:             result,
		expectedTags:       newTags,
		expectedTimestamps: expectedTimestamps,
		autoGenerated:      !args.withProvidedTimestamps,
	})
}
...
func verifyServiceTagRemoved(args *verifyServiceTagRemovedParams) {
	args.params.t.Helper()

	// Verify the removed tag is no longer present
	found := slices.ContainsFunc(args.result.Attributes.Properties,
		func(prop *feature.TagProperty) bool {
			return prop.Tag == args.removedTag &&
				prop.Value == args.removedValue
		})
	args.params.assert.False(
		found,
	...
	args.params.assert.Equal(
		args.originalTagCount-1,
		len(args.result.Attributes.Properties),
...
func testRemoveTagsSuccess(params *testParams) {
	...
	verifyServiceTagRemoved(&verifyServiceTagRemovedParams{
		params:           params,
		result:           result,
		removedTag:       "remove_me",
		removedValue:     "remove_value",
		originalTagCount: originalTagCount,
	})
...
func testRemoveTagsSingleTag(params *testParams) {
	...
	verifyServiceTagRemoved(&verifyServiceTagRemovedParams{
		params:           params,
		result:           result,
		removedTag:       "single_remove",
		removedValue:     "single_value",
		originalTagCount: originalTagCount,
	})
...
// createFeatureWithPartialMatchTags sets up a feature with tags that have the same name but different values.
func createFeatureWithPartialMatchTags(
	params *testParams,
	featureID string,
) (*feature.FeatureAnnotation, int) {
	params.t.Helper()
...
// verifyPartialTagRemoval verifies that only the specified tag/value was removed,
// while preserving other tags with the same name.
func verifyPartialTagRemoval(
	params *testParams,
	result *feature.FeatureAnnotation,
	originalTagCount int,
) {
...
func testRemoveTagsPartialMatch(params *testParams) {
	params.t.Helper()
	updated, originalTagCount := createFeatureWithPartialMatchTags(
		params,
		"DDB_G0285704",
	)
	...
	params.assert.NoError(
		err,
		"should successfully remove specific tag/value",
	)

	// Verify the removal was correct
	verifyPartialTagRemoval(params, result, originalTagCount)
}
